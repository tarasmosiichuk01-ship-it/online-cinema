...
async def test_register_user_success(client, db_session):
    ...
    query_token = select(ActivationToken).where(
...
async def test_activate_account_success(client, db_session, seed_user_groups):
    ...
    activation_response = await client.get(
        f"/api/v1/accounts/activate/{user.activation_token.token}/"
    )
    assert (
...
async def test_activate_user_with_expired_token(client, db_session, seed_user_groups):
    ...
    token = ActivationToken(
        user_id=user.id, expires_at=datetime.now(timezone.utc) - timedelta(days=2)
    )
...
async def test_activate_already_active_user(
    client, db_session_commit, seed_user_groups
):
    """
...
async def test_request_password_reset_token_success(
    ...
    reset_response = await client.post(
        "/api/v1/accounts/forgot-password/", json=reset_payload
    )
    assert (
    ...
    assert (
        ...
        == "If you wish to reset your password, you will receive an email."
...
async def test_request_password_reset_token_nonexistent_user(client, db_session):
    ...
    query = (
        ...
        .join(User)
        .where(User.email == reset_payload["email"])
    )
    result = await db_session.execute(query)
...
async def test_reset_password_success(client, db_session_commit, seed_user_groups):
    ...
    stmt_reset = select(PasswordResetToken).where(
        PasswordResetToken.user_id == created_user.id
    )
...
async def test_reset_password_invalid_email(client, db_session):
    ...
    """
    reset_payload = {"new_password": "NewTest1234!", "confirm_password": "NewTest1234!"}

    response = await client.post(
        "/api/v1/accounts/reset-password/random_token/", json=reset_payload
    )

    assert response.status_code == 400, "Expected status code 400 for invalid email."
    assert (
        response.json()["detail"] == "Invalid or expired password reset token."
...
async def test_reset_password_invalid_token(client, db_session, seed_user_groups):
    ...
    response = await client.post(
        "/api/v1/accounts/forgot-password/", json=reset_request_payload
    )
    ...
    response = await client.post(
        f"/api/v1/accounts/reset-password/{invalid_token}/", json=reset_complete_payload
    )
    ...
    ), "The original valid token should still exist in the DB."
...
async def test_reset_password_expired_token(
    ...
    reset_response = await client.post(
        f"/api/v1/accounts/reset-password/{token_value}/", json=reset_complete_payload
    )
    assert (
        reset_response.status_code == 400
    ), "Expected status code 400 for expired token."
    assert (
        reset_response.json()["detail"] == "Invalid or expired password reset token."
    ), "Unexpected error message."

    query_token_check = select(PasswordResetToken).where(
        PasswordResetToken.user_id == user_id
    )
...
async def test_reset_password_sqlalchemy_error(
    ...
    with patch("routes.accounts.AsyncSession.commit", side_effect=SQLAlchemyError):
        reset_response = await client.post(
            f"/api/v1/accounts/reset-password/{token_value}/",
...
async def test_login_user_success(
    client, db_session_commit, jwt_manager, seed_user_groups
):
    """
    ...
    """
    user_payload = {"email": "login_testuser@example.com", "password": "Test1234!"}
...
async def test_login_user_inactive_account(client, db_session_commit, seed_user_groups):
    ...
    """
    user_payload = {"email": "inactive_testuser@example.com", "password": "Test1234!"}
...
async def test_refresh_access_token_user_not_found(
    ...
    corrupted_refresh_token = jwt_manager.create_refresh_token(
        {"user_id": invalid_user_id}
    )

    refresh_token_record = RefreshToken.create(
        user_id=user.id, days_valid=7, token=corrupted_refresh_token
    )
