...
@pytest.mark.asyncio
async def test_add_movie_to_cart_get_carts_delete_movie_from_cart_check_cart(
    ...
    assert (
        get_cart_items_response.json()["cart_items"][0]["movie"]["name"]
        == test_movie.name
    )

    deleted_item_response = await client.delete(
...
@pytest.mark.asyncio
async def test_add_movies_to_cart_clear_cart_check_empty(
    authorized_client, test_movie, db_session_commit
):
    """
    ...
    first_response = await client.post(
        "/api/v1/shopping_carts/carts", json={"movie_id": test_movie.id}
    )
    assert first_response.status_code == 201

    second_response = await client.post(
        "/api/v1/shopping_carts/carts", json={"movie_id": second_movie.id}
    )
