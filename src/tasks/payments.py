...
def add_movies_to_purchased_table(
    ...
    with SyncSessionLocal() as session:
        try:
            query = select(OrderItem.movie_id).where(OrderItem.order_id == order_id)
            ...
            purchased_movies_data = [
                {"user_id": user_id, "movie_id": movie_id} for movie_id in movie_ids
            ]
            ...
            session.execute(
                delete(CartItem).where(CartItem.cart_id == cart_id_subquery)
            )

            session.commit()
            logger.info(
                f"Successfully moved {len(movie_ids)} movies to Purchased for user {user_id}"
            )
